...
// Ici, on importe le package HTTP natif de Node et l'utilisez pour créer un serveur, en passant une fonction qui sera exécutée à chaque appel effectué vers ce serveur. Cette fonction reçoit les objets request et response en tant qu'arguments. Dans cet exemple,
...
const normalizePort = val => { // config du port d'écoute des requettes . renvoie un port valide, qu'il soit fournit sous la forme d'un numéro ou d'une chaîne ;
...
const server= http.createServer(app) ;// on crée serveur node  avec l'argument app de express.js
