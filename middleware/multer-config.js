...
const storage = multer.diskStorage({
  ...
  filename: (req, file, callback) => {
    const name = file.originalname.split(' ').join('_'); // on retire tous les espaces du nom du fichier et on les remplace par des underscores pour éviter des erreurs
    const extension = MIME_TYPES[file.mimetype];
    callback(null, name + Date.now() + '.' + extension); // composition du nouveau nom du fichier avec le timestamp actuel
  }
});

module.exports = multer({storage: storage}).single('image'); // single indique qu'on recoit un seul fichier à la fois
